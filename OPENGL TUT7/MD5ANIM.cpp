...
bool MD5Anim::Init(const char* filename)
{
...
mat4* MD5Anim::GetAnimatedMatrices(){ return m_pAnimatedMatrices; }

bool MD5Anim::LoadAnim(const char* filename)
{
	...
	while (!file.eof())
	{
		...
		else if(param == "numJoints")
		{
			...
			m_pAnimatedMatrices = new mat4[m_iNumJoints];
			if(!m_pAnimatedMatrices)
				return false;
								 ...
								 md5_frame_data_t* frameDataArray)
{
	...
	for(int i = 0; i < m_iNumJoints; i++)
	{
		...
		memcpy(animjoint.position, baseframe.position, sizeof(float3));
		memcpy(animjoint.orientation, baseframe.orientation, sizeof(float4));
		...
		if(animjoint.parentId >= 0)
		{
			...
			float3 rotpos;
			mlRotateVec3ByQuaternion(rotpos, parentjoint.orientation, animjoint.position);
			...
			mlMultiplyQuaternion_2(animjoint.orientation, parentjoint.orientation, animjoint.orientation);
			mlNormalizeVec4(animjoint.orientation, animjoint.orientation);
...
{
	mat4 finalmatrix, rotation, translation;
	...
	for(int i = 0; i < m_iNumJoints; i++)
	{
		...
		mlQuaternionToMat4(rotation, finaljoint.orientation);
