...
public:
	...
	void ProcessKeyboard(Camera_Movement direction, float deltaTime) {
		...
		if (direction == C_REINICIO){
			this->CharPosition = glm::vec3(0.0f, -2.0f, 0.0f);
			this->Front = glm::vec3(0.0f, 0.0f, -1.0f);
			this->Yaw = YAW;
			this->Pitch = PITCH;
			this->distanceFromPlayer = DISTANCE;
			this->angleAroundPlayer = 0.0f;
			Colision::colisionCroc = false;
			Colision::colisionTrex = false;
		...
		{
			Colision::bajarCaja = true;
		}
		if (direction == C_CajaReles)
		{
			Colision::bajarCaja = false;
		}
		if (direction == C_FORWARD && !Colision::colisionCroc && !Colision::colisionTrex && !Colision::colisionPumpkin){
			this->CharPosition += this->Front * velocity;
			this->CamPosition += this->Front * velocity;
		}
		if (direction == C_BACKWARD && !Colision::colisionCroc && !Colision::colisionTrex && !Colision::colisionPumpkin){
			this->CharPosition -= this->Front * velocity;
			this->CamPosition -= this->Front * velocity;
		}
		if (direction == C_LEFT && !Colision::colisionCroc && !Colision::colisionTrex && !Colision::colisionPumpkin){
			this->angleAroundPlayer -= 0.1f ;
			this->calcCameraPositionDirection();
		}
		if (direction == C_RIGHT && !Colision::colisionCroc && !Colision::colisionTrex && !Colision::colisionPumpkin){
			this->angleAroundPlayer += 0.1f ;
			this->calcCameraPositionDirection();
		}
