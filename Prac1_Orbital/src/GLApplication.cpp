...
void GLApplication::applicationLoop() {
	...
	while (processInput) {
		...
		/*Cambiar el timeValue por una variable, relacionada con una tecla
		haría que se moviera una cantidad controlada de movimientos*/

		glm::vec3 cameraPos = windowManager->inputManager.getCameraPos();
		
		
		//sin((GLfloat)timeValue * 1.0f) * radius;
		/*
		GLfloat radius = 10.0f;
		float camX = radius * sin(windowManager->inputManager.getCameraPos()) 
			float camZ = radius * cos(windowManager->inputManager.getCameraPos()
			+ windowManager->inputManager.getCameraDirection();)
			*/
		
		
		glm::vec3 cameraLookAt = windowManager->inputManager.getCameraDirection();
			//+ windowManager->inputManager.getCameraDirection();


		//view = glm::translate(view,
			//glm::vec3(-cameraPos.x, -cameraPos.y, -cameraPos.z));
			view = glm::lookAt((glm::vec3(camX, 0.0f, camZ)+posicionObjeto[0]), posicionObjeto[0], glm::vec3(0, 1, 0));
